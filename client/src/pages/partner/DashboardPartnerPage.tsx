import { CalendarCheck, DollarSign, Package, ShoppingCart, TrendingUp } from "lucide-react";
import TitleRoutesAdminPartner from "../../components/TitleRoutesAdminPartner";
import { useSummaryPartner } from "../../hooks/dashboard/Partner/useSummaryAdmin";
...
const DashboardPartnerPage = () => {
    const { data: summaryData } = useSummaryPartner();
    const navigate = useNavigate();

    const stats = [
        { label: 'Pesanan Hari Ini', value: summaryData?.order_today, change: '+12%', icon: CalendarCheck, color: 'bg-blue-500', link: '/partner/pesanan' },
        { label: 'Total Pesanan', value: summaryData?.order, change: '+8%', icon: ShoppingCart, color: 'bg-green-500', link: '/partner/pesanan' },
        { label: 'Dompet', value: formatRupiah(summaryData?.wallet ?? 0), change: '+23%', icon: DollarSign, color: 'bg-[#F26A24]', link: '' },
        { label: 'Produk Aktif', value: summaryData?.product_active, change: '+5%', icon: Package, color: 'bg-purple-500', link: 'partner/produk' },
    ];
